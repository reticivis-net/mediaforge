...
from mainutils import improcess, ytdownload, tenorsearch, prefix_function
...
class Conversion(commands.Cog, name="Conversion"):
    ...
    async def icon(self, ctx, *, body=None):
        ...
        """
        if body is None:
            result = [await improcessing.iconfromsnowflakeid(ctx.author.id, self.bot, ctx)]
        else:
            id_regex = re.compile(r'([0-9]{15,20})')
            tasks = []
            for m in re.finditer(id_regex, body):
                tasks.append(improcessing.iconfromsnowflakeid(int(m.group(0)), self.bot, ctx))
