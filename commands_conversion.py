...
import utils_discordmisc
from clogs import logger
from mainutils import improcess, tenorsearch, prefix_function
from utils_dpy import UnicodeEmojiConverter
...
class Conversion(commands.Cog, name="Conversion"):
    ...
    async def icon(self, ctx, *, body=None):
        ...
        """
        if body is None:
            result = [await utils_discordmisc.iconfromsnowflakeid(ctx.author.id, self.bot, ctx)]
        else:
            id_regex = re.compile(r'([0-9]{15,20})')
            tasks = []
            for m in re.finditer(id_regex, body):
                tasks.append(utils_discordmisc.iconfromsnowflakeid(int(m.group(0)), self.bot, ctx))
        ...
        try:
            ...
            if r:
                ...
                if not r:
                    return
                txt = ""
                vcodec = await processing_ffprobe.get_vcodec(r)
                acodec = await processing_ffprobe.get_acodec(r)
    ...
    async def togif(self, ctx):
        ...
        """
        await improcess(ctx, processing_ffmpeg.mp4togif, [["VIDEO"]])
    ...
    async def toapng(self, ctx):
        ...
        """
        await improcess(ctx, processing_ffmpeg.toapng, [["VIDEO", "GIF"]], resize=False)
    ...
    async def toaudio(self, ctx):
        ...
        :mediaparam video: A video.
        """
        await improcess(ctx, processing_ffmpeg.toaudio, [["VIDEO", "AUDIO"]])
    ...
    async def tovideo(self, ctx):
        ...
        """
        await improcess(ctx, processing_ffmpeg.giftomp4, [["GIF"]])
    ...
    async def topng(self, ctx):
        ...
        """
        await improcess(ctx, processing_ffmpeg.mediatopng, [["VIDEO", "GIF", "IMAGE"]])
