...
class Debug(commands.Cog, name="Owner Only", command_attrs=dict(hidden=True)):
    ...
    async def errorcmd(self, _):
        ...
        """
        await processing_common.run_command("ffmpeg", "-hide_banner", "dsfasdfsadfasdfasdf")
    ...
    async def checkwatermark(self, ctx):
        ...
        if file:
            ...
            result = await processing_other.checkwatermark(file[0])
            if result:
