...
from utils_dpy import UnicodeEmojiConverter, showcog
from mainutils import improcess, imagesearch, saveurls, prefix_function

from utils_dpy import add_long_field
...
class Other(commands.Cog, name="Other"):
    ...
    async def addemoji(self, ctx, name):
        ...
        """
        await improcess(ctx, utils_discordmisc.add_emoji, [["GIF", "IMAGE"]], ctx.guild, name, expectresult=False,
                        resize=False)
    ...
    async def addsticker(self, ctx, stickeremoji: UnicodeEmojiConverter, *, name: str):
        ...
        :mediaparam media: A gif or image.
        """
        await improcess(ctx, utils_discordmisc.add_sticker, [["GIF", "IMAGE"]], ctx.guild, stickeremoji, name,
    ...
    async def setbanner(self, ctx):
        ...
        if "BANNER" not in ctx.guild.features:
            ...
            return
        await improcess(ctx, utils_discordmisc.set_banner, [["IMAGE"]], ctx.guild, expectresult=False,
                        resize=False)
    ...
    async def setservericon(self, ctx):
        ...
        """
        await improcess(ctx, utils_discordmisc.set_icon, [["IMAGE", "GIF"]], ctx.guild, expectresult=False,
                        resize=False)
    ...
    async def version(self, ctx):
        ...
        :param ctx: discord context
        """
        await processing_common.run_command("git", "fetch")
        status = await processing_common.run_command("git", "status")
    ...
    async def ffmpegversion(self, ctx):
        ...
        :param ctx: discord context
        """
        status = await processing_common.run_command("ffmpeg", "-version")
        with io.StringIO() as buf:
            buf.write(status)
            buf.seek(0)
    ...
    async def info(self, ctx):
        ...
        if file:
            ...
            result = await processing_ffprobe.ffprobe(file[0])
